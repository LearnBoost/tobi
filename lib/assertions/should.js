...
Assertion.prototype.__defineGetter__('selected', function(){
  var elem = this.obj
    , selected = elem.get(0).getAttribute('selected');

  this.assert(
      'selected' == selected
    , 'expected ' + j(elem) + ' to be selected'
    , 'expected ' + j(elem) + ' to not be selected');

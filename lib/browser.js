...
Browser.prototype.parse = function(html){
  ...
  // TODO: move out
  this.jQuery.fn.select = function(options){
    console.log(options);
  };
...
Browser.prototype.select = function(locator, options){
  var $ = this.jQuery
    , elem = this.locate('select', locator);

  $(elem).find('option').filter(function(i, option){
...
Browser.prototype.fill = function(fields){
  ...
  for (var locator in fields) {
    var val = fields[locator]
      , elem = this.locate('select, input, textarea', locator)[0];
    switch (elem.nodeName) {
      case 'SELECT':
        $(elem).select(val);
        break;
...
/**
 * Ensure `html` is wrapped with <html></html> tags.
 *
 * @return {String}
 * @api public
 */
